...
while [[ $# -gt 0 ]]; do
    case "$2" in
        develop)
            git clone --branch=master --depth 100 "https://x-token-auth:${REPO_ACCESS_TOKEN}@bitbucket.org/powtoon/gitops-dev.git" gitops/
            cd gitops/
            ...
            ;;
        staging-ec)
            git clone --branch=master --depth 100 "https://x-token-auth:${REPO_ACCESS_TOKEN}@bitbucket.org/powtoon/gitops-ec-staging.git" gitops/
            cd gitops/
            ...
            ;;
        prod-ec)
            git clone --branch=master --depth 100 "https://x-token-auth:${REPO_ACCESS_TOKEN}@bitbucket.org/powtoon/gitops-ec-prod.git" gitops/
            cd gitops/
