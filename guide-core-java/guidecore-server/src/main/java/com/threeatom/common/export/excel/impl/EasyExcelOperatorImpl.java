...
public class EasyExcelOperatorImpl implements ExcelOperator {
    ...
    public <T> void writeExcelToWebResponse(
    ...
    private List<List<Object>> createDynamicModelList(
        ...
        for (StudentBehaviorDataExcel studentBehaviorDataExcel : studentBehaviorExcelData) {
            ...
            if (studentBehaviorDataExcel.getWatchedNum() != null
                    && studentBehaviorDataExcel.getWatchedNum() != null) {
            ...
            if (null != studentBehaviorDataExcel.getSubjectList()) {
                for (int i = 0; i < studentBehaviorDataExcel.getSubjectList().size(); i++) {
                    row.add(
                            WatchedStatusType.getByCode(
                                            studentBehaviorDataExcel.getSubjectList().get(i).getWatchedStatus())
                                    .getDesc());
                    if (Objects.nonNull(studentBehaviorDataExcel.getSubjectList())
                            && studentBehaviorDataExcel.getSubjectList().size() != 0) {
                        if (null != studentBehaviorDataExcel.getSubjectList().get(i).getSubjects()) {
                            studentBehaviorDataExcel
                                    .getSubjectList()
                                    .get(i)
                                    .getSubjects()
                                    .forEach(
