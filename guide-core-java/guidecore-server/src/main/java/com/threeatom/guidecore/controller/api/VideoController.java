...
public class VideoController {
    ...
    @PostMapping("/{videoId}/tasks")
    @ResponseStatus(HttpStatus.CREATED)
    public ResponseEntity<List<TaskDto>> videoTasks(@PathVariable Integer videoId,
                                                    @RequestBody @Valid com.threeatom.guidecore.dto.request.TaskDto taskDto,
                                                    HttpServletRequest request) {
        List<TaskDto> tasks = videoEventFacade.createTask(taskDto, videoId, getPortalUser(request));
