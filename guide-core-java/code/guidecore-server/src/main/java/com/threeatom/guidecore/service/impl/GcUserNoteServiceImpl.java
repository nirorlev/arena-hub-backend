 ...
 */
@Service
public class GcUserNoteServiceImpl extends ServiceImpl<GcUserNoteMapper, GcUserNote> implements GcUserNoteService {

    private static final Logger LOGGER = LoggerFactory.getLogger(GcUserNoteServiceImpl.class);

    @Override
    public List<GcUserNote> selectNoteByUserId(Integer userId, Integer videoId, Integer masterId, HttpServletRequest request) {
	...
	@Override
	public List<GcUserNote> selectNoteListByUserMaster(Integer userId, Integer masterId,HttpServletRequest request){
        PageParam pageParam = new PageParam(request);
        Integer pageNum = pageParam.getPageNum();
        Integer pageSize=pageParam.getPageSize();
        if (pageNum > 0 && pageSize > 0) {
