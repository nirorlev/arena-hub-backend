 ...
 */
@Service
public class GcEventServiceImpl extends ServiceImpl<GcEventMapper, GcEvent> implements GcEventService {

    @Autowired
    private GcVideoService videoService;

    @Autowired
    private GcUserEventService gcUserEventService;

    @Autowired
    private GcUserEventMapper gcUserEventMapper;
        ...
        try {
            ...
            if(event.getUploadType()!=null&&event.getUploadType()==2){
                ...
                if(Objects.nonNull(event.getClassId())){
                    ...
                    userIds = userAccessList.stream().map(GcUserAccess::getUserId).collect(Collectors.toList());
                }else {
    ...
    public JSONArray getEventListAndSelfAnswerByVid(Integer vid, Integer userId) {
		...
		for (GcEvent event:eventList) {
			List<GcEvent> newEvent = answerList.stream().filter(a->a.getId().equals(event.getId())).collect(Collectors.toList());
			if(newEvent.size() > 0)
				reList.add(newEvent.get(0));
			else
				reList.add(event);
            ...
            List<GcUserEventResource> userEventResourceList = userEventResourceService.getEventResourceByEventIdAndUserId(event.getId(),userId);
    ...
    @Override
    public List<GcEvent> selectGetEventListAndSelfAnswerByVidFull(Integer vid, Integer userId,Integer masterId){
        List<GcEvent> list =  this.baseMapper.selectGetEventListAndSelfAnswerByVidFull(vid, userId,masterId);
        ...
        if(list.size()!=0) {
            ...
            list.forEach(i -> {
                ...
                if (null != map) {
                    List<Integer> userLists = Arrays.stream(Arrays.asList(map.get("userList").toString().split(",")).stream().mapToInt(Integer::parseInt).toArray()).boxed().collect(Collectors.toList());
...
//        return this.list(queryWrapper);
     ...
     */
    @Override
    public List<GcEvent> findEventAnswerByVideoIdsUser(List<Integer> videoIds, Integer userId,Integer masterId,Integer envFlag) {
        return this.baseMapper.findEventAnswerByVideoIdsUser(videoIds, userId,masterId,envFlag);
    }

    @Override
    public List<GcEvent> findEventUserByVideoIdsUser(List<Integer> videoIds, Integer userId, Integer masterId, Integer envFlag) {
    ...
    @Override
    public Map<Integer,Object> videoEventsAnswerNumMap(Integer vid, Integer userId,Integer masterId){
    ...
    @Override
    public List<GcEvent> findEventAnswerByVideoIdsUserList(List<Integer> videoIds, List<Integer> userId, Integer masterId) {
    ...
    @Override
    public List<GcEvent> selectEventByPermissionList(List<Integer> permissionList, List<Integer> userIdList, Integer masterId) {
        return this.baseMapper.selectEventByPermissionList(permissionList,masterId,userIdList);
    }

    @Override
    public List<GcEvent> selectEventByUserIdAndSubjectId(Integer userId,Integer subjectId,Integer masterId) {
        return this.baseMapper.selectEventByUserIdAndSubjectId(userId,subjectId,masterId);
    }

    @Override
    public List<GcEvent> selectEventBySubjectIdUserIds(List<Integer> userIds,Integer subjectId,Integer masterId){
