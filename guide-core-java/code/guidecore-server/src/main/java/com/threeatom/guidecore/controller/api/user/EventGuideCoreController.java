...
public class EventGuideCoreController extends GuideCoreController {
    ...
    public Message eventAnswerList(@PathVariable("eventId") Integer eventId, HttpServletRequest request) {
        //test
        ...
        List<GcUserAnswer> eventAnswerList = userAnswerService.getAnswerListByEventId(eventId,null,masterId);
        ...
        List<GcUserEventResource> userEventResourceList = userEventResourceService.getEventResourceByEventIdAndUserId(eventId,user.getId());
        //20200220修改
    ...
    public Message answerQuestion(@RequestBody JSONObject jsonRequest, HttpServletRequest request) {
        return gvgMasterService.answerQuestion(jsonRequest,request,request.getIntHeader("masterId"),this.getGcUser(),EnvType.GC.getCode(),this.getSystem());
    }

    @ApiOperation(value = "保存问题资源统一接口-新", httpMethod = "POST")
    @PostMapping("/saveEventResourceCommon")
    public Message saveEventResourceCommon(@RequestBody GcUserEventResource userEventResource,HttpServletRequest request) {
//    	eventId
//    	type (原resType)
//    	targetUserId
//    	targetId
...
//    	fileId(新增)
    	...
    	userEventResource.setTimeNode(userEventResource.getTimeNode()==null?-1:userEventResource.getTimeNode());
    	...
    	if(userEventResource.getTargetId()!=null&&userEventResource.getTargetId()>0) {
        	GcUserEventResource eventResource=userEventResourceService.getById(userEventResource.getTargetId());
        	userEventResource.setTargetUserId(eventResource.getUserId());
        }else if(null!=userEventResource.getTargetUserIdList() && TableConstant.COMMON_ZERO!=userEventResource.getTargetUserIdList().size()){
    	    //usage页面批量保存通知
    	...
    	if(Objects.isNull(userEventResource.getTargetUserId()) && CollectionUtils.isNotEmpty(userEventResource.getTargetUserIdList())){
    	    //批量feedback
        ...
        if (userEventResource.getTargetUserId() == null) {
            ...
            if(TableConstant.COMMON_ZERO!=userEventResourceList.size()){
                ...
                for(Integer userId : userEventResource.getTargetUserIdList()){
                    ...
                    Optional<GcUserEventResource> gcUserEventResource =userEventResourceList.stream().filter(e->e.getTargetUserId().equals(userId)).findFirst();
            ...
            }else if (CollectionUtils.isEmpty(userEventResource.getTargetUserIdList()) && sendIds.size() > 0) {
