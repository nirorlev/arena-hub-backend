...
public class NewUISaveContentController extends GuideCoreController {
	...
	public Message contentFolderList(HttpServletRequest request) {
		...
		List<GcUserSaveFolder> list = gcUserSaveFolderService.selectFolderForUserMaster(user.getId(), getHeaderMasterId(request),null,request,null);
		for(GcUserSaveFolder folder: list) {
			for(GcUserSaveContent content: folder.getSaveContentList()) {
				if(content.getVideo()!=null)sysFileService.getVideoSnapshotUrl(content.getVideo().getVideoFile());
	...
	public Message ptContentFolderList(HttpServletRequest request) {
		...
		if(CollectionUtils.isNotEmpty(gcUserSaveContentFollowIdList)) {
			List<GcUserSaveFolder> followedplaylist = gcUserSaveFolderService.selectFolderForUserMaster(null, getHeaderMasterId(request), gcUserSaveContentFollowIdList, request,null);
			for(GcUserSaveFolder gcUserSaveFolder : followedplaylist){
				...
				List<GcUserSaveContent> videoContents = contents.stream().filter(e->e.getFileId()!=null).collect(Collectors.toList());
		...
		//recommenplaylist
		List<GcUserSaveFolder> recommentPlayList = gcUserSaveFolderService.selectFolderInMaster(getHeaderMasterId(request));
		List<Integer> recommenFolderIds = recommentPlayList.stream().map(GcUserSaveFolder::getId).collect(Collectors.toList());
		List<GcUserSaveFolder> recommenFolderList = gcUserSaveFolderService.selectFolderForUserMaster(null, getHeaderMasterId(request),recommenFolderIds,request,listIds);
...
//			if(CollectionUtils.isNotEmpty(gcUserSaveFolder.getSaveContentList())) {
		...
		List<GcMasterHomeInfo> allHomeInfos = iGcMasterHomeInfoService.getGcMasterHomeInfoList(request.getIntHeader("masterId"),TableConstant.gcMasterHomeInfo_name_homepage_list,getSystem(),request);
		m.ok().addData("homeInfos",allHomeInfos);
		return m;
	}

	@PostMapping("/saveDeletePlayListFollow")
	public Message savePlayListFollow(@RequestBody GcUserSaveContentFollow gcUserSaveContentFollow, HttpServletRequest request) {
		Message message = new Message();
		GcUserSaveContentFollow gcUserSaveContentFollow1 = gcUserSaveContentFollowService.selectFollowByUserId(this.getGcUser().getId(),gcUserSaveContentFollow.getFolderId());
...
//	public Message deletePlayListFollow(@RequestBody GcUserSaveContentFollow gcUserSaveContentFollow, HttpServletRequest request) {
//		GcUserSaveContentFollow gcUserSaveContentFollow1 = gcUserSaveContentFollowService.selectFollowByUserId(this.getGcUser().getId(),getHeaderMasterId(request));
//		if(gcUserSaveContentFollowService.removeById(gcUserSaveContentFollow1)) {
...
//	}
	...
	public Message saveContentToFolder(@RequestBody GcUserSaveContent gcUserSaveContent, HttpServletRequest request) {
		ApiAssert.notNull(gcUserSaveContent.getFolderId(), "文件夹id不可空");
		if(gcUserSaveContent.getVideoId()!=null && gcUserSaveContent.getSubId()!=null)throw new SystemException("课程id及视频id不可同时存在，一次仅可保存其中一个");
		...
		if(count>0)return new Message().error(101,I18NUtil.get("guidecore.user.saveContentRepeat"));
	...
	public Message getContentFromOneFolder(@RequestBody GcUserSaveFolder gcUserSaveFolder, HttpServletRequest request) {
		GcUser user = this.getGcUser();
		return gcMasterService.getContentFromOneFolder(gcUserSaveFolder,user,request, EnvType.GC.getCode());
	}
	...
	public Message deleteFolder(Integer folderId, HttpServletRequest request) {
		...
		gcUserSaveFolder.setUserId(this.getGcUser().getId());
		if(gcUserSaveFolderService.countFolder(gcUserSaveFolder)==0)return new Message().error("该用户folderId不存在记录");
	...
	public Message deleteSaveContent(Integer contentId, HttpServletRequest request) {
		...
		gcUserSaveContent.setUserId(this.getGcUser().getId());
		if(gcUserSaveContentService.countSaveContent(gcUserSaveContent)==0)return new Message().error("该用户contentId不存在记录");

		if (gcUserSaveContentService.removeById(contentId))
			return new Message().ok("删除成功");
		else
			return new Message().error("删除失败");
	}
