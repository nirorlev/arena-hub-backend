...
import com.threeatom.common.userdata.RedisUserInfo;
 ...
 */
@Service
public class GcAccessServiceImpl extends ServiceImpl<GcAccessMapper, GcAccess> implements GcAccessService {

    @Autowired
    GcUserAccessService userAccessService;

    @Resource
    private NewUiGcSubjectMapper newUiGcSubjectMapper;
    @Autowired
    ...
    @Autowired
    private MondayConfiguration mondayConfiguration;
    @Autowired
    @Lazy
    ...
    public boolean addAccess(GcAccess access) {
        ...
        if(access.getId()!=null) {
            ...
            List<Integer> userAccessIds=list.stream().map(GcUserAccess::getId).collect(Collectors.toList());
...
//        queryWrapper.isNull("package_name");
    ...
    @Override
    public List<GcAccess> listAccess(String name,Integer masterId,Integer userId, HttpServletRequest request) {
        return this.baseMapper.listAccess(name,masterId,userId);
    }

    @Override
    public List<GcAccess> getTeamAccessList(String name, Integer masterId, Integer userId, HttpServletRequest request) {
    ...
    @Override
    public List<GcAccess> getTeamAccessSubjectNumList(String name, Integer masterId, Integer userId, List<Integer> subIds,HttpServletRequest request) {
    ...
    private List<GcAccess> listToTree(List<GcAccess> gcAccessList){
        ...
        Map<String,GcAccess> gcAccessMap = gcAccessList.stream().collect(Collectors.toMap(GcAccess::getCode, (p) -> p));
    ...
    @Transactional
    @Override
    public Message checkUserAccess(Integer masterId, Integer userId, String accessCode, Integer inviteUserId, GcUser user, HttpServletRequest request) throws ClientException, IOException {
        // TODO Auto-generated method stub
        QueryWrapper<GcAccess> queryWrapper = new QueryWrapper<GcAccess>();
...
//        if(userAccess != null)return message.ok("Already have your code in this portal");
        if (userAccess == null) {
            ...
            if(null!=user) {
                ...
                //String textBody = I18NUtil.get("guidecore.master.portalEmailText") + gcMaster.getContext();
                String email = user.getUsername();
                List<String> emailList = JSONObject.parseArray(gcMaster.getEmailCc().toJSONString(),String.class);
                emailList.add(email);
                String htmlBody = I18NUtil.get("guidecore.master.email")
                        .replace("{user.name}",user.getFirstName()+user.getLastName())
                        .replace("{email}",user.getUsername())
                        .replace("{url}",request.getHeader("origin"));
               // SingleSendMailResponse response = emailService.sendEmail(emailService.getLoginMasterEmail(emailList,htmlBody));
            }
...
//                mondayApiVo.setInvoiceUrl(charge.getReceiptUrl());
        ...
        if(inviteUserId!=null&&inviteUserId!=0) {
            GcUserAccess inviteUserAccess=userAccessService.getUserAccessByMasterIdAndUserId(masterId, inviteUserId);
    ...
    @Override
    public List<GcAccess> getAllPackage(Integer masterId, PageParam pageParam,List<Integer> packageIdList,List<Integer> idList) {
        ...
        List<GcAccess> allPackage = this.baseMapper.getAllPackage(masterId,showFlag,packageIdList,idList);
        return allPackage;
    }
