...
public class NewUIUserController extends GuideCoreController {
    ...
    public Message getHomeData(HttpServletRequest request) {
    	...
    	GcUserAccess userAccess = userAccessService.getUserAccessByMasterIdAndUserId(masterId, user.getId());
    ...
    public Message  videoEventDetail(@RequestBody JSONObject jsonRequest, HttpServletRequest request) {
        ...
        if(null==master.getAnswerShowFlag() || TableConstant.COMMON_ONE==master.getAnswerShowFlag()) {
            eventAnswerList = userAnswerService.getAnswerListByEventId(eventId, this.getGcUser().getId(), masterId);
        }else {
...
//        }
    ...
    public Message InteractionsEventAnswerList(@RequestBody JSONObject jsonRequest, HttpServletRequest request) {
        Message message = new Message();
		Integer eventId = jsonRequest.getInteger("eventId");
		Integer masterId = request.getIntHeader("masterId");
...
//        List<GcEvent> portalEventsList = gcEventService.selectEventUploadByPortal(videoId);
        ...
        //课程IDlist
		List<Integer> gcEventIds = gcEventList.stream().map(GcEvent::getId).collect(Collectors.toList());
        ...
        if(gcEventIds.size()!=TableConstant.COMMON_ZERO) {
            //查询所有问题下的回答
            if(null==gcMaster.getAnswerShowFlag() || TableConstant.COMMON_ONE==gcMaster.getAnswerShowFlag()) {
                eventAnswerList = userAnswerService.getAnswerListByEvent(gcEventIds, masterId, user.getId());
            } else {
                eventAnswerList = userAnswerService.getMyAnswerListByEvent(gcEventIds,masterId,user.getId());
            }
            //按照问题分类
            ...
            if(jsonRequest.getInteger("studentId")==null) {
                ...
                unReadMessages = gcUserEventResourceService.selectUnCheckedTeacherMessage(user.getId(), gcEventIds,masterId);
            }else {
                //如果studentid不为空，代表是教师用户，查找学生发送的消息
                unReadMessages = gcUserEventResourceService.selectUnCheckedStudentMessage(jsonRequest.getInteger("studentId"),gcEventIds,user.getId(),masterId);
            }
            //根据问题分类
            resMessageList = unReadMessages.stream().collect(Collectors.groupingBy(GcUserEventResource::getEventId));
            //计算问题下回答总数以及未读消息数量
            for (GcEvent event1 : gcEventList) {
                ...
                if(null!=gcUserAnswers) {
                    ...
                    Optional<GcUserAnswer> myOptionAnswer =gcUserAnswers.stream().filter(e ->user.getId().equals(e.getUserId())).findFirst();
                    //判断当前学生用户是否回答过这个问题
                    if(myOptionAnswer.isPresent()){
                        ...
                        if(event1.getEventType().equals(TableConstant.COMMON_ONE)) {
                            ...
                            if (JSONObject.parseArray(jsonArray.toJSONString(), String.class).containsAll(rightAnswer) && rightAnswer.containsAll(JSONObject.parseArray(jsonArray.toJSONString(), String.class))) {
                ...
                if(null!=gcUserEventResources) {
                    List<GcUserEventResource> unreadMessagesFromTeacherList = gcUserEventResources.stream().filter(e->null!=e.getReadState()).filter(e -> e.getReadState() == TableConstant.COMMON_ZERO).collect(Collectors.toList());
                    event1.setUnReadMessages(unreadMessagesFromTeacherList.size());
                }
            }
            //按照eventtime顺序排序问题
            List<GcEvent> orderByEventTimeSubjectList = gcEventList.stream().sorted(Comparator.comparing(GcEvent::getEventTime)).collect(Collectors.toList());
...
//                }
//            }
        List<GcEvent> answeredEvents = gcEventList.stream().filter(e->e.getThisUserAnsweredOrNot()==TableConstant.COMMON_ONE).collect(Collectors.toList());
    ...
    public Message changeUsername(@RequestBody GcUser gcUser, HttpServletRequest request) throws Exception {
...
//        if(null!=masterIdList && masterIdList.size()>TableConstant.COMMON_ZERO && masterIdList.contains(gcUser.getSubjectMasterId())){
//            return new Message().error("the user does not belong to this master");
//        }
        if(TableConstant.COMMON_ONE==gcManager.getSuperAdminFlag()){
            return new Message().ok().addData("",this.gcUserService.updateById(gcUser)).addData("newEmail",gcUser);
        }else{
                List<GcUserAccess> gcUserAccessList = gcUserAccessService.getAccessListByUser(gcUser.getId());
                List<Integer> masterIdList = gcUserAccessList.stream().map(GcUserAccess::getMasterId).collect(Collectors.toList());
    ...
    public Message videoNoteList(@RequestBody JSONObject jsonRequest, HttpServletRequest request) {
    	...
    	ApiAssert.notNull(videoId, "视频id不可空");
    	 ...
    	 List<GcUserNote> userNoteList = userNoteService.selectNoteByUserId(this.getGcUser().getId(),videoId,masterId,request);
         for (GcUserNote note : userNoteList) {
        	 if(note.getFile()!=null)
             sysFileService.getResFullUrl(note.getFile(), request);
    ...
    public Message deleteVideoNote(Integer noteId, HttpServletRequest request) {
    	...
    	if(userNoteService.countNoteByNote(note)==0)return new Message().error("该用户没有该笔记id权限，或该id不存在");
    	
    	 if (userNoteService.removeById(noteId))
    ...
    public Message selectNoteListByUserMaster(HttpServletRequest request) {
    	List<GcUserNote>  list = userNoteService.selectNoteListByUserMaster(this.getGcUser().getId(),getHeaderMasterId(request),request);
    	PageInfo<GcUserNote> pageInfo = new PageInfo<>(list);
    	return new Message().ok("删除成功").addData("list", pageInfo).addData("返回说明", "id：视频id，fileNum：文件数量，videoName：视频名称");
    }

    @ApiOperation(value = "视频打星", httpMethod = "POST")
    @PostMapping("/putStarValue")
    public Message putStarValue(@RequestBody GcUserVideoAction userVideoAction, HttpServletRequest request) {
        ...
        GcUserVideoAction action = gcUserVideoActionService.getActionByAction(userVideoAction);//查看是否打过星
    ...
    public Message changeUserInfo(@RequestBody GcUserInfo info, HttpServletRequest request) {
    	...
    	if(username!=null) {
        	...
        	if(existEmailUser!=null) throw new SystemException(I18NUtil.get("guidecore.user.emailExist"));
        	GcUser user = this.getGcUser();
    ...
    public Message changePassword(@RequestBody JSONObject requestParams, HttpServletRequest request) throws ClientException {
        GcManager gcManager = this.getManager();
        GcUser user = this.getGcUser();
    	...
    	String pwdHash = new SimpleHash("MD5", newPassword, user.getSalt() + SysConstant.PASS_SALT).toHex();
    ...
    public Message selectComment(HttpServletRequest request,Integer eventId,Integer otherUserId){
        Integer masterId = request.getIntHeader("masterId");
        List<UserNoteCommentVo> voList = gcUserNoteCommentService.selectNoteComment(eventId,otherUserId,request);
        return new Message().ok().addData("list",voList);
    }

    @ApiOperation(value = "保存回答问题下评论", httpMethod = "POST")
    @PostMapping("/saveNoteComment")
    public Message saveNoteComment(HttpServletRequest request,@RequestBody GcUserNoteComment userNoteComment){
        GcUser user = this.getGcUser();
        Integer masterId = request.getIntHeader("masterId");
