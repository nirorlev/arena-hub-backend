...
public class SysFileServiceImpl extends ServiceImpl<SysFileMapper, SysFile> implements SysFileService {
    ...
    private Optional<String> getVideoPlayerUrlFromExternalVideo(SysFile sysFile) {
        ...
        try {
            ...
            return Optional.ofNullable(videoData.getString("playerUrl"));
    ...
    public String getS3Url(String S3ObjectKey) {
        ...
        try {
            ...
            signedUrl = CloudFrontService.signUrl(param_UrlToBeSigned, awsUploadSignUrlConfiguration.getKeyPairId(),
                derPrivateKey, policy);
        } catch (Exception e) {
    ...
    public SysFile saveRes(Integer uploaderId, SysSystem sys, String folder, Integer saveType, String oriFileName,
                           MultipartFile file) {
        String fileName = UUID.randomUUID().toString() + FileUtil.getExtensionName(file.getOriginalFilename());
        AliyunOssService ossService = this.getCurrentOssService(sys);
        InputStream is = null;
    ...
    public JSONObject saveFilePolicy(Integer uploaderId, SysSystem sys, String folder, Integer saveType,
                                     String originFileName, Map<String, Object> extParams) {
    ...
    public String getVideoSnapshotUrl(SysFile sysFile) {
        ...
        switch (sysFile.getSaveType()) {
            case DISK_SAVE_TYPE:
                if ((null != sysFile.getFileTypeIndex() && sysFile.getFileTypeIndex().equals(EventUnifyType.AUDIO_3)) ||
                    (null != sysFile.getFolder() &&
                        sysFile.getFolder().equals(TableConstant.sysFile_folder_guidecoreAudio))) {
                    break;
                }
                String objectName = sysFile.getFileUrl();
                videoSnapshotUrl =
                    ossService.getVideoSnapshot(ossService.getCurrentBucketName(), objectName, 10000);//第3秒为视频预览图
                sysFile.setSnapshotUrl(videoSnapshotUrl);
                break;
            case FILE_SAVE_TYPE:
                break;
    ...
    public String getVideoSnapshotUrl(GcVideo gcVideo) {
        ...
        switch (videoFile.getSaveType()) {
            case DISK_SAVE_TYPE:
                if ((null != videoFile.getFileTypeIndex() &&
                    videoFile.getFileTypeIndex().equals(EventUnifyType.AUDIO_3)) || (null != videoFile.getFolder() &&
                    videoFile.getFolder().equals(TableConstant.sysFile_folder_guidecoreAudio))) {
                    break;
                }
    ...
    public JSONObject saveFilePolicyMD5(Integer uploaderId, SysSystem sys, String folder, Integer saveType,
                                        String originFileName, String md5, Map<String, Object> extParams) {
    ...
    @Override
    public List<SysFile> getFiles(String folder, Integer masterId, List<Integer> typeIndexIds, String tag,
                                  Integer pageNum, Integer pageSize, String searchString, Integer fileId,
                                  Integer uploadUid) {
        if (pageNum == null) {
            pageNum = 0;
        }
        ...
        return ((SysFileMapper) this.baseMapper).selectFilesByTypeIndexAndTagSearch(folder, masterId, tag, typeIndexIds,
            searchString, fileId, uploadUid);
    }

    public List<String> getAllTag(Integer masterId, SysSystem sys, List<String> folders) {
        Map<String, Object> map =
            ((SysFileMapper) this.baseMapper).selectFileTagJSONStrByFolder(masterId, sys.getId(), folders);
