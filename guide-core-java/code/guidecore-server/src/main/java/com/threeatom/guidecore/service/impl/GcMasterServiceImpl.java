 ...
 */
@Service
public class GcMasterServiceImpl extends ServiceImpl<GcMasterMapper, GcMaster> implements GcMasterService {
	@Resource
	NewUIUserMapper newUIUserMapper;

	@Autowired
	private SysFileService sysFileService;

	@Resource
	GcMasterMapper gcMasterMapper;
...
//	}
	...
	@Override
	public Message getContentFromOneFolder(@RequestBody GcUserSaveFolder gcUserSaveFolder,GcUser user, HttpServletRequest request,Integer envFlag) {
...
//		List<GcSubject> oneList = new ArrayList<>();
		...
		try{
			...
			if(Objects.nonNull(user)){
				content = new GcUserSaveContent(user.getId(),gcUserSaveFolder.getMasterId(),gcUserSaveFolder.getId());
			}else {
				content = new GcUserSaveContent(null,gcUserSaveFolder.getMasterId(),gcUserSaveFolder.getId());
			}
			...
			if(envFlag.equals(EnvType.PT.getCode())) {
				...
				if (null!=gcUserInfo.getAvatarFileId()){
					...
					gcUserInfo.getAvatarFile().setFullFileUrl(sysFileService.getResFullUrl(gcUserInfo.getAvatarFile(),request));
				}
			...
			if (oneSubIdList.size()!=0){
				...
				newUiGcSubjectService.buildSubject2(oneList,user.getId(),new SysSystem(),request,EnvType.GC.getCode());
			...
			//获取视频
			List<GcUserSaveContent> list = gcUserSaveContentService.selectContetnByFolderId(content.getFolderId());
			List<Integer> fileIds = this.gcUserSaveContentService.getVideoIdList(content);
			if (null!=fileIds&&fileIds.size()!=0){
				//videoList = gcVideoService.selectVideoAndTimeByIds(videoIdList,new PageParam(request),request.getIntHeader("masterId"));
			...
			if(Objects.nonNull(gcUser)) {
				List<Integer> follows = gcUserSaveContentFollowService.selectFollowPlayList(user.getId(), request.getIntHeader("masterId"));
