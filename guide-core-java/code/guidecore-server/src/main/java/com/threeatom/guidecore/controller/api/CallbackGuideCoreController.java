...
public class CallbackGuideCoreController extends GuideCoreController {
    ...
    public Message aliyunOssCallback(@RequestBody JSONObject jsonObject) {
        ...
        if (uploadType != null && uploadType.equals(UploadType.portalFrontPageVideo_2)) {
            ...
            gcMaster
                    .getIntroVideoFile()
                    .setFullFileUrl(fileService.getResFullUrlSaveType2(gcMaster.getIntroVideoFile()));

            masterService.updateSourceNull(
                    gcMaster
                            .getId()); // source_type：null或1:本地intro_video_id不为空，2:youku，3:screenRock，2和3时intro_video_id为空，source_url不为空
            return new Message().ok().addData("gcMaster", gcMaster);
        }

        // Case 3: 作业本回复答案的文件上传
        if (uploadType != null && uploadType.equals(UploadType.workbookUplodFile_3)) {
            GcUserEventResource userEventResource =
                    userEventResourceService.uploadEventResourceFile(jsonObject);
